...
class StringMatchingTest(unittest.TestCase):
    ...
    def tearDownClass(cls) -> None:
        ...
        print(
            f"'어린 왕자' 단어가 나타난 pattern string start indice: {get_string_matching_result(cls.str_data, '어린 왕자')}"
        )
    ...
    def test_string_matching(self):
        self.assertEqual(
            get_string_matching_result(StringMatchingTest.str_data, "어린 왕자"),
            [
                match.start()
                for match in re.finditer(r"어린 왕자", StringMatchingTest.str_data)
            ],
        )
...
![](https://res.cloudinary.com/gyunseo-blog/image/upload/f_auto/v1701261814/image_b7zkq9.png)
