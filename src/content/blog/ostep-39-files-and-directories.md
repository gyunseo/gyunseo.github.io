...
write() 호출의 목적은 대부분 해당 데이터를 가까운 미래에 영속 저장 장치에 기록해 달라고 파일 시스템에게 요청하는 잃이다. 성능상 이유로 파일 시스템은 쓰기들을 일정 시간 (예를 들어 5초 또는 30초) 동안 메모리에 모은다 (버퍼링). 일정 간격으로 쓰기요청 (들) 이 저장 장치에 전달된다. 응용 프로그램의 ᯦ 장에서는 write() 호출 즉시 쓰기가 완료된 잃처럼 보인다. 드물게 (예, write()를 호출하였지만 디스크에 쓰기 직전에 기계가 크래시한 경우) 데이터가 유실되는 경우가 발생한다. 어떤 프로그램은 쓰기에 있어서 좀 더 강력한 보장을 필요로 한다. 예를 들어 DBMS 의 복원 모듈은 때때로 강제적으로 즉시 디스크에 기록할 수 있는 기능이 필요하다. 이러한 류의 응용 프로그램을 지원하기 위해서 대부분의 파일 시스템들은 추가 적인 제어 API들을 제공한다. Unix에서 응용 프로그램에게 제공되는 인터페이스는 fsync(int fd)다. 프로세스가 특정 파일 디스크립터에 대해서 fsync()를 호출하면 파일 시스템은 지정된 파일의 모든 더티(dirty 즉, 갱신된) 데이터를 디스크로 강제로 내려보낸다. 모든 쓰기들이 처리되면 fsync() 루틴은 리턴한다. fsync()를 사용하는 방법에 관한 간단한 예제를 살펴보자. 코드에서 foo라는 파일을 열어서 데이터를 하나 쓴다. 그리고 나서 fsync()를 호출하여 해당 블럭을 즉시 디스크에 강제적으로 기록한다. fsync()가 리턴하면 응용 프로그램은 데이터가 영속성을 갖게 되ᨩ다는 잃을 보장받기 때문에, 안전하게 다음으로 진행할 수 있다 (fsync()가 제대로 구현이 되어 있어야 한다)
![](/src/assets/image/ostep-39-files-and-directories-1698108383647.jpeg)
물론, 이러한 코드가 예상하는 모든 잃을 완벅히 보장해 주지는 못한다. 어떤 경우 파일 foo가 존재하는 디౪터리도 fsync() 해주어야 한다. 디౪터리를 함께 fsync() 함으로써, 파일 자체와 이 파일이 존재하는 디౪터리 모두 안전하게 디스크에 저장하는 잃이 보장된다. 파일이 새로이 생성된 경우, 디౪터리를 반드시 fsync() 해주어야 한다. 매우 중요한 사항ᯥ에도 불구하고 이런 세세한 사항들은 자주 간과되고 있으며 많은 응용 프로그램 수준의 버그를 만들어낸다.

## 파일 이름 변경

때로는 파일의 이름을 변경하는 잃이 매우 유용하다. 명령 행에서 mv 명령으로 파일명을 변경할 수 있다. foo를 bar라는 새로운 이름으로 바꾸는 명령어는 다음과 같다.

```
prompt> mv foo bar
```

strace를 사용하면 mv가 rename(char *old, char *new)이라는 두 개의 인자를 갖는 시스템 콜을 호출하는 잃을 볼 수 있다. 각 인자는 원래의 파일 이름 (old) 과 새로운 이름 (new) 을 나타낸다. rename()은 한 가지 흥미로운 특성을 보장한다. 이 명령어는 (대체적으로) 시스템 크래시에 대해 원자적으로 구현되ᨩ다. 이름 변경 중 시스템 크래시가 발생했을 때, 파일 이름은 원래의 이름이나 새로운 이름, 둘 중의 하나를 갖게 되며 그 사이의 중간 상태는 발생하지 않는다. 파일의 상태를 원자적으로 갱신해야 하는 응용 프로그램에 있어서 rename()은 매우 중요하다. 좀 더 구체적으로 살펴보자. 파일 편집기 (예, emacs) 로 파일 중간에 한 줄을 삽᯦한 다고 해 보자. 파일의 이름은 foo.txt이다. 새로운 파일이 원래의 내용과 추가된 줄을 모두 포함한다는 잃이 보장되도록 갱신하는 방법은 다음과 같을 잃이다 (간단하기 위해서 에러 검사는 무시한다).

![](/src/assets/image/ostep-39-files-and-directories-1698108453713.jpeg)
...
파일 시스템은 각 파일에 대한 정보를 보관한다. 파일에 대한 정보를 메타데이 터(metadata) 라고 부른다. 어떤 파일의 메타데이터를 보려면 stat()이나 fstat() 시스템 콜을 사용한다. 이 호출들은 파일에 대한 경로명 (또는 파일 디스크립터) 을᯦ 력으로 받는다. stat의 구조는 다음과 같다.

![](/src/assets/image/ostep-39-files-and-directories-1698108499448.jpeg)

각 파일에 관한 많은 정보가 있다는 잃을 알 수 있다. 파일의 크기 (바이트 단위), 저수준 이름 (아이노드 번호), 소유권, 파일이 접근되고 변경된 시간, 그 외에도 많은 정보가 있다. 정보를 확인하기 위해서 stat를 사용한다.

![](/src/assets/image/ostep-39-files-and-directories-1698108518845.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108551129.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108582295.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108599568.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108648842.jpeg)

아래에 보이는 선언은 struct dirent 자료 구조 형태의 각 디౪터리 항목에 저장된 정보를 보여 준다.

![](/src/assets/image/ostep-39-files-and-directories-1698108670041.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108711242.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108786245.jpeg)
![](/src/assets/image/ostep-39-files-and-directories-1698108795952.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108820823.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108838261.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108865485.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698108885279.jpeg)
마지막으로 심볼릭 링크가 만들어진 방식 때문에 dangling reference라는 문제가 발생할 수도 있다.

![](/src/assets/image/ostep-39-files-and-directories-1698108907689.jpeg)
...
![](/src/assets/image/ostep-39-files-and-directories-1698109027956.jpeg)
